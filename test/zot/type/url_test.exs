defmodule Zot.Type.URLTest do
  ...
  describe "host validation" do
    test "accepts URI with host" do
      assert {:ok, "https://example.com"} = Z.url() |> Z.parse("https://example.com")
    end

    test "rejects relative path (no host)" do
      assert {:error, [issue]} = Z.url() |> Z.parse("/relative/path")
      assert Exception.message(issue) == "host is required"
    end

    test "rejects URN (no host)" do
      assert {:error, [issue]} = Z.url() |> Z.parse("urn:isbn:0451450523")
      assert Exception.message(issue) == "host is required"
    end
  end
  ...
  describe "require_path" do
    ...
    test "accepts URI with non-root path when required" do
      assert {:ok, "https://example.com/foo"} =
               Z.url(require_path: true) |> Z.parse("https://example.com/foo")
    end

    test "rejects URI with nil path when required" do
      assert {:error, [issue]} =
               Z.url(require_path: true) |> Z.parse("https://example.com")
    ...
    test "accepts nested path when required" do
      assert {:ok, "https://example.com/foo/bar"} =
               Z.url(require_path: true) |> Z.parse("https://example.com/foo/bar")
    end
  end
  ...
  describe "allowed_ports" do
    test "passes when URI has no explicit port" do
      assert {:ok, "https://example.com"} =
               Z.url(allowed_ports: [80, 443]) |> Z.parse("https://example.com")
    end

    test "accepts port in allowed list" do
      assert {:ok, "https://example.com:8080/path"} =
               Z.url(allowed_ports: [80, 8080]) |> Z.parse("https://example.com:8080/path")
    end

    test "rejects port not in allowed list" do
      assert {:error, [issue]} =
               Z.url(allowed_ports: [80, 443]) |> Z.parse("https://example.com:9090/path")
  ...
  describe "forbidden_ports" do
    test "passes when port is not in forbidden list" do
      assert {:ok, "https://example.com:8080/path"} =
               Z.url(forbidden_ports: [25]) |> Z.parse("https://example.com:8080/path")
    end

    test "rejects port in forbidden list" do
      assert {:error, [issue]} =
               Z.url(forbidden_ports: [25]) |> Z.parse("https://example.com:25/path")
    ...
    test "passes when URI has no explicit port" do
      assert {:ok, "https://example.com"} =
               Z.url(forbidden_ports: [25]) |> Z.parse("https://example.com")
    end
  end
  ...
  describe "allow_loopback" do
    test "default (true) allows loopback addresses" do
      assert {:ok, _} = Z.url() |> Z.parse("https://localhost/path")
      assert {:ok, _} = Z.url() |> Z.parse("https://127.0.0.1/path")
    end

    test "rejects localhost when disallowed" do
      assert {:error, [issue]} =
               Z.url(allow_loopback: false) |> Z.parse("https://localhost/path")
    ...
    test "rejects subdomain of localhost when disallowed" do
      assert {:error, [issue]} =
               Z.url(allow_loopback: false) |> Z.parse("https://foo.localhost/path")
    ...
    test "rejects 127.x.x.x when disallowed" do
      assert {:error, [issue]} =
               Z.url(allow_loopback: false) |> Z.parse("https://127.0.0.1/path")

      assert Exception.message(issue) == "loopback addresses are not allowed"

      assert {:error, [issue]} =
               Z.url(allow_loopback: false) |> Z.parse("https://127.255.0.1/path")
    ...
    test "rejects ::1 when disallowed" do
      assert {:error, [issue]} =
               Z.url(allow_loopback: false) |> Z.parse("https://[::1]/path")
    ...
    test "allows non-loopback addresses when disallowed" do
      assert {:ok, "https://example.com/path"} =
               Z.url(allow_loopback: false) |> Z.parse("https://example.com/path")
    end

    test "allows non-loopback IPs when disallowed" do
      assert {:ok, "https://8.8.8.8/path"} =
               Z.url(allow_loopback: false) |> Z.parse("https://8.8.8.8/path")
    end
  end
end
